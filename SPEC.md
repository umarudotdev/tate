...
Construction fails if the path is empty, the symbol name is empty, or the format is unrecognized. All downstream code works with `Entry`, never with `String`.

**Grade** (parsed from keypress, never stored as raw integer internally):

```rust
enum Grade {
    Blank,  // SM-2 quality 1
    Hard,   // SM-2 quality 3
    Good,   // SM-2 quality 4
    Easy,   // SM-2 quality 5
}
...
struct New {
    due: NaiveDate,  // today for fresh cards, tomorrow after failed review
...
Card<New>      --[grade == 1]--> Card<New>         (stays new, due tomorrow)
...
| ---------- | ----- | ---------------- | --------------------------------- |
| 1          | Blank | 1                | No recall. Complete failure.      |
...
| Extension(s)   | Grammar crate            | Symbol node types                                                                                                                             |
| -------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `.ts`, `.tsx`  | `tree-sitter-typescript` | `function_declaration`, `class_declaration`, `interface_declaration`, `type_alias_declaration`, `lexical_declaration` (const arrow functions) |
| `.js`, `.jsx`  | `tree-sitter-javascript` | `function_declaration`, `class_declaration`, `lexical_declaration` (const arrow functions)                                                    |
| `.py`          | `tree-sitter-python`     | `function_definition`, `class_definition`                                                                                                     |
| `.rs`          | `tree-sitter-rust`       | `function_item`, `struct_item`, `enum_item`, `impl_item`, `trait_item`, `type_item`                                                           |
...
| `.c`, `.h`     | `tree-sitter-c`          | `function_definition`, `struct_specifier`, `enum_specifier`                                                                                   |
| `.cpp`, `.hpp` | `tree-sitter-cpp`        | `function_definition`, `class_specifier`, `struct_specifier`                                                                                  |
| `.rb`          | `tree-sitter-ruby`       | `method`, `class`, `module`                                                                                                                   |
| `.sql`         | `tree-sitter-sql`        | Whole-file only, no symbol resolution                                                                                                         |

Unsupported extensions: whole-file tracking only, no `::Symbol` entries allowed.
...
Questions are provided externally, not generated by Tate. The AI agent that wrote the code provides the question at add-time via the `-q` flag. Tate has no LLM dependency.

### Adding questions

```
tate add src/auth/login.ts::authenticate -q "What happens when the token is expired but the refresh token is still valid?"
```

If `-q` is used on an entry that already exists in the deck, the question is updated (entry is not re-added).

Questions are stored in the `questions` table keyed by entry. The current `body_hash` is stored alongside the question.
...
**Usage:** `tate add <path>` or `tate add <path>::<Symbol>`

**Flags:** `-q <question>` (optional) provides a decompression question for review sessions.
...
enum Message {
    ...
    QuestionLoaded(Entry, Option<String>),              // question read (or not)
...
- Grade input: single keypress (1-4), no enter required. Invalid keys are ignored (terminal adapter loops).
- `q`: produces `Message::Quit`. The core transitions to session summary.
- Progress on completed cards is saved. Remaining cards are still due.
