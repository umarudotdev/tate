...
mod tests {
    ...
    fn new_blank_stays_new() {
        let result = sm2_update(new_card(0), Grade::Again, today(), 365);
    ...
    fn learning_blank_lapses_to_new() {
        let result = sm2_update(learning_card(2, 6, 0), Grade::Again, today(), 365);
        if let TypedCard::New(c) = result {
    ...
    fn mature_blank_lapses_to_new() {
        let result = sm2_update(mature_card(5, 30, 2), Grade::Again, today(), 365);
        if let TypedCard::New(c) = result {
    ...
    fn ease_clamping_after_repeated_blanks() {
        ...
        for _ in 0..50 {
            card = sm2_update(card, Grade::Again, today(), 365);
        }
